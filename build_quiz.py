...
def parse_docx(path: Path) -> dict:
    ...
    for i, raw in enumerate(paras):
        ...
        if m:
            # ── Bug fix 2026-06-26: docx paragraph contains "解析：..." + the next
            # question's "X．【题型】" merged together (typical of <w:br/> soft breaks
            # joining two paragraphs into one). Symptom: next question's stem starts
            # with "解析：" and the previous question's analysis is empty.
            # Fix: peel off the 解析 prefix and assign it to the previous cur, but
            # STOP at the next "X．" question-number marker so the prefix is preserved.
            #
            # PARSE_RE itself is greedy (`[^\n]*` swallows to EOL including the
            # question number), so we use a custom non-greedy regex that explicitly
            # stops at the next "X．" pattern (or end of string).
            prefix_text = s[:m.start()]
            parse_prefix_m = re.match(
                r'^解析\s*[：:]\s*(.*?)(?=\s*\d+\s*[.．、]|\Z)',
                prefix_text,
                re.DOTALL,
            )
            if parse_prefix_m and cur is not None:
                parse_content = parse_prefix_m.group(1).strip()
                strip_to = parse_prefix_m.end()
                parse_lines.append(parse_content)
                commit_parse()  # flush immediately to cur['analysis']
                s = s[strip_to:].lstrip()
                m = Q_TYPE_RE.search(s)  # re-locate Q marker in stripped s
                if m is None:
                    qtype_early = None  # let later OPT/ANSWER/PARSE branches recover
                else:
                    qtype_early = m.group(1)
            else:
                qtype_early = m.group(1)  # used by the multi-line sub-loop below
            # 检查段内是否含 \n 嵌入了选项
            if '\n' in s:
        ...
        if ans_m and cur is not None and cur['answer'] is None:
            ...
            # 同一段可能紧跟 \n 解析, 处理它
            rest = s[ans_m.end():]
            # ── Bug fix 2026-06-26: "答案：B\n解析：..." 时 rest 仍以 "B" 开头,
            # 旧版 ^解析 正则漏匹配. 改为遍历 rest 各行找首个"解析：".
            parse_inline = None
            for line in rest.split('\n'):
                line = line.strip()
                if not line:
                    continue
                pm = re.match(r'^解析\s*[：:]\s*([^\n]+)', line)
                if pm:
                    parse_inline = pm
                    break
            if parse_inline:
    ...
    with open(csv_path, 'w', encoding='utf-8', newline='') as f:
        ...
        for qset in final_quizzes:
            for q in qset['questions']:
                ...
                # CSV stem column: strip the leading "X．" / "X." / "X、" question
                # number — the number is already captured in the q_no column, and
                # downstream consumers (SQLite/MySQL import) don't want it duplicated.
                csv_stem = re.sub(r'^\s*\d+\s*[.．、]\s*', '', q['stem']).strip()
                w.writerow([
                    qset['number'],
                    qset['title'],
                    q['number'],
